...
const RemovePrivateModal = ({ projectId, setIsPrivate }: ModalProps) => {
  ...
  const submitHandler = async (e:  React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const res = await axios.patch(`/api/projects/${projectId}/removePrivate`, { password });
      if (res.status === 200) {
        toast.success("Your file is now public.");
        setIsPrivate(false);
        close();
      } else {
          ...
          >
            <div className="flex flex-col mx-2">
              ...
              <p className="px-6 mx-2 mt-2 text-gray-500">
                Remove restricted access so anyone can view or edit this file.
              </p>
